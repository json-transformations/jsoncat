...
def test_no_args_with_stdin(runner):
    result = runner.invoke(jsoncat.cli.main, args=[], input=J1)
...
def test_single_filename(runner):
    with runner.isolated_filesystem():
        with open('t1.json', 'w') as f:
            f.write(J1)
        result = runner.invoke(jsoncat.cli.main, args=['t1.json'])
        assert result.exit_code == 0
        assert eval(result.output) == D1
...
def test_stdin_arg_plus_filename(runner):
    with runner.isolated_filesystem():
        with open('t2.json', 'w') as f:
            f.write(J2)
        result = runner.invoke(jsoncat.cli.main, args=['-', 't2.json'],
                               input=J1)
        assert result.exit_code == 0
...
def test_two_filenames(runner):
    with runner.isolated_filesystem():
        ...
        with open('t2.json', 'w') as f:
            f.write(J2)
        result = runner.invoke(jsoncat.cli.main, args=['t1.json', 't2.json'])
        assert result.exit_code == 0
        assert eval(result.output) == [D1, D2]
...
def test_two_filenames_reversed(runner):
    with runner.isolated_filesystem():
        ...
        with open('t2.json', 'w') as f:
            f.write(J2)
        result = runner.invoke(jsoncat.cli.main, args=['t2.json', 't1.json'])
        assert result.exit_code == 0
...
def test_compact_json(runner):
    result = runner.invoke(jsoncat.cli.main, args=['--compact'], input=J1)
    assert result.exit_code == 0
    assert len(result.output.splitlines()) == 1


def test_json_decode_error(runner):
    result = runner.invoke(jsoncat.cli.main, args=[], input=JE)
    assert result.exit_code == 1


def test_no_args_without_stdin(runner):
    result = runner.invoke(jsoncat.cli.main, args=[])
    assert result.exit_code == 1
...
def test_no_args_with_no_stdin_and_is_tty():
    ...
    with pytest.raises(SystemExit):
        jsoncat.core.load_json_files(ctx=ctx, is_tty=True)
